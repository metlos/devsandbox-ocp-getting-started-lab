...
`oc delete all --selector app=datasphere-ui -n {lab-namespace}` to clean up, then re-run the
...
`oc delete all --selector app=datasphere-ui -n {lab-namespace}` to clean up, then try again.
====
