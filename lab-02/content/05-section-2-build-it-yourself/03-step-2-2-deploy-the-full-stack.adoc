...
----
helm install datasphere {gitea-root-url}/api/packages/gitea-admin/helm/datasphere-{datasphere_helm_version}.tgz --set api.hpa.enabled=false --set api.probes.enabled=false --set api.resources.enabled=false
----
...
// SCREENSHOT NEEDED: Import YAML editor (⊕ button) in {lab-namespace} with DataSphere manifests pasted in, Create button visible at bottom
